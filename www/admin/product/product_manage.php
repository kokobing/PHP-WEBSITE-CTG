...
//交换位置
	if(isset($_GET["action"]) && $_GET["action"]=="switch"){
		...
		$objDB->Execute($strSQL);
	    header('Location:product_manage.php?&searchselect=&searchdate=&keywords=&page='.$_GET[page]);ob_end_flush();exit();//跳转
	}
...
if(isset($action) && $action=="02" && $onuserperm_addprem=='1'){
    ...
    header('Location:product_manage.php'); ob_end_flush(); exit();
...
if(isset($action) && substr($action,0,2)=="04" && $onuserperm_editprem=='1'){
	...
	header('Location:product_manage.php?03'.$oneproductinfoid.'&searchselect='.$searchselect.'&keywords='.$keywords.'&modify=1'); ob_end_flush(); exit();
